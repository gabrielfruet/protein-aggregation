...
_aminoacids_to_num = {aa: i for i, aa in enumerate(_aminoacids)}

_aminoacids_from_num = {i: aa for i, aa in enumerate(_aminoacids)}
...
def aa_to_num(seq: Union[str, list[str]]) -> list[int]:
    if type(seq) is list:
        for e in seq:
            if len(e) > 1:
                raise ValueError(
                    "Sequence should be either a string or an\
